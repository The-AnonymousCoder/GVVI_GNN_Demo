...
对于一个约百万参数以内的图模型，这不是极小样本。当前主要问题是：已有73维特征大多是距离、数量、坐标和平均高度统计，同一个空间位置附近即使有大量观察点，模型仍不知道：
...
因此模型的上限主要由“特征是否包含决定GVVI的关键信息”决定，而不是CSV、图片或压缩包有多少GB。

## 1.2 本轮明确不下载新数据

本轮优化只使用仓库现有数据，不再下载图片、OBJ、点云、DSM或其他数据包。

原因：

- 当前节点和观察点数量足够完成同区补全实验；
- 主要问题是现有数据尚未被充分转化为有效特征；
- 新增GB级文件会增加下载、清洗、坐标对齐和排错时间；
- 颜色ID图、可见像素表等数据还可能造成标签泄漏；
- 局部几何数据若不能覆盖固定测试集，会破坏评价公平性。

DeepSeek不得以“需要更多数据”为由暂停当前优化，也不得自行下载官方大文件。优化范围固定为：

```text
充分利用现有观察点表
→ 构造方向对齐和距离衰减特征
→ 利用WVI建筑ID、SVI道路ID、DVI高度层
→ 将训练标签比例从70%提高到80%
→ 构建多尺度残差GraphSAGE
→ 优化零值、正值和综合GVVI损失
```

如果最终仍未达到 `R² ≥ 0.50`，如实记录“缺少建筑遮挡和真实视线信息”这一局限，但本轮不通过下载新数据继续扩展。
